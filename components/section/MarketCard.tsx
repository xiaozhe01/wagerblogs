  ...
  return (
    <section className="flex flex-col gap-3" aria-labelledby="market-at-a-glance">
      ...
      <dl className="grid grid-cols-2 md:grid-cols-4 gap-legacy-4 md:gap-3">
        {marketStats.map((s) => (
          <div key={s.label} className="card text-center flex flex-col">
            <dt className="order-2 text-xs text-text-meta leading-snug">{s.label}</dt>
            <dd className="order-1 text-3xl font-bold text-text-primary mb-1">{s.value}</dd>
            <dd className="order-3 text-2xs text-text-subtle font-mono mt-1.5 leading-snug">
              {s.source}
            </dd>
